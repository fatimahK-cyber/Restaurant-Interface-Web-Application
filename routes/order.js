...
router.get('/management', (req, res, next) => { // Step 2: Fetch the each ordered menu item name, quantity, and order ID from the database.
  const get_order_totals_query = `SELECT mi.Name, oi.Order_ID, oi.Quantity FROM Menu_Item AS mi JOIN Order_Item AS oi ON mi.Menu_Item_ID=oi.Menu_Item_ID ORDER BY Order_ID;`;
...
/*
PATCH /order/management middleware chain:
